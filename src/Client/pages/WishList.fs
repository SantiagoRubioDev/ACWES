...
let update (msg:WishListMsg) model : Model*Cmd<WishListMsg> = 
    ...
    | LinkChanged link -> 
        { model with NewBook = { model.NewBook with Link = link }; LinkErrorText = WishListValidation.verifyBookLink link }, Cmd.none
    | RemoveBook book -> 
    ...
    | AddBook ->
        if WishListValidation.verifyBook model.NewBook then
