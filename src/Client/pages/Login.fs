    ...
    promise {
        ...
        try
            ...
            if not response.Ok then
                ...
                let data = res.Split(' ')
                return (data.[0],data.[1]) //(token,userType)
        with
...
let init (user:UserData option) = 
    ...
    | Some user ->
        { Login = { UserName = user.UserName; Password = ""}
          State = LoggedIn (user.Token,user.UserType)
          ErrorMsg = "" }, Cmd.none

let update (msg:LoginMsg) model : Model*Cmd<LoginMsg> = 
    match msg with
    | LoginMsg.GetTokenSuccess (token,userType) ->
        { model with State = LoggedIn (token,userType); Login = { model.Login with Password = "" } }, []
...
let view model (dispatch: AppMsg -> unit) = 
    ...
    | LoggedOut ->
        div [ClassName "signInBox" ] [
          h3 [ ClassName "text-center" ] [ text "Log in with your IC ID"]
